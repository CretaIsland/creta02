...
class GaugeView extends StatefulWidget {
  ...
  const GaugeView(
      ...
      : assert(alertSpeedArray.length == alertColorArray.length,
        ...
        super(key: key);
  @override
  GaugeViewState createState() => GaugeViewState(/*speed, animate*/);
}

class GaugeViewState extends State<GaugeView> with SingleTickerProviderStateMixin {
  ...
  GaugeViewState(/*this._speed, this._animate*/);

  @override
  void initState() {
    logger.info('GuageView initState()');
    if (widget.animate) {
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        updateSpeed(/*widget.speed, animate: widget.animate*/);
      });
    } else {
      lastMarkSpeed = widget.speed;
      _gaugeMarkSpeed = widget.speed;
    }
    ...
    _animation = Tween(begin: 0.0, end: 1.0).animate(_controller)
      ..addListener(() {
        setState(() {
          _gaugeMarkSpeed = lastMarkSpeed + (widget.speed - lastMarkSpeed) * _animation.value;
        });
      })
  ...
  void updateSpeed(/*double speed, {bool animate = false, Duration? duration}*/) {
    logger.info('updateSpeed(${widget.speed})');
    if (widget.animate) {
      //widget.speed = speed;
      _controller.reset();
...
class _GaugeCustomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0; 270 >= i; i = i + 45) {
      canvas.drawCircle(_getDegreeOffsetOnCircle(mDottedCircleRadius, i + arcStartAngle),
          minDimension * .012, paint);
    }
    ...
    for (int i = 0; alertSpeedArray.length > i; i++) {
      paint.color = alertColorArray[i];
      canvas.drawCircle(
          _getDegreeOffsetOnCircle(
              mDottedCircleRadius, _getAngleOfSpeed(alertSpeedArray[i]) + arcStartAngle),
          minDimension * .015,
          paint);
    }
  ...
  void _drawMinText(Canvas canvas, Size size) {
    TextSpan span =
        TextSpan(style: minMaxTextStyle, text: minSpeed.toStringAsFixed(fractionDigits));
  ...
  void _drawMaxText(Canvas canvas, Size size) {
    TextSpan span =
        TextSpan(style: minMaxTextStyle, text: maxSpeed.toStringAsFixed(fractionDigits));
    TextPainter textPainter = TextPainter(
      text: span,
      textDirection: TextDirection.ltr,
    );
  ...
  void _drawUnitOfMeasurementText(Canvas canvas, Size size) {
    ...
    unitOfMeasurementOffset =
        unitOfMeasurementOffset.translate(-textPainter.width / 2, -textPainter.height / 2);
